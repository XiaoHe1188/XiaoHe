$(document).ready(function() {
	...
	$('body').on('click', '.template-skins > a', function(e) {
		e.preventDefault();
		var skin1 = $(this).data('skin');
		sessionStorage.setItem("skin", skin1);
		$('body').attr('id', skin1);
	...
	(function() {
		setTimeout(function() {
			...
			$('.todo-list .media .checked').each(function() {
				$(this).closest('.media').find('.checkbox label').css('text-decoration',
					'line-through')
			});
	...
	(function() {
		...
		if ($('[class*="color-picker"]')[0]) {
			$('[class*="color-picker"]').colorpicker().on('changeColor', function(e) {
				...
				$(this).closest('.color-pick').find('.color-preview').css('background', e.color
					.toHex());
			});
		}
	})();
	...
	(function() {
		$('.tab-vertical').each(function() {
			...
			var tabContentHeight = $(this).closest('.tab-container').find('.tab-content')
				.outerHeight();
